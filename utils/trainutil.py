...
def get_CosineAnnealingLR(optim, conf):
    return torch.optim.lr_scheduler.CosineAnnealingLR(optim, T_max=conf.epochs, eta_min=0, last_epoch=-1)
...
def get_params(model,conf=None):
    ...
    if conf is not None and 'prams_group' in  conf:
        ...
        for pram,lr in zip(prams_group,lr_group):
            ...
            # params.append({'params':model.get_params(pram),'lr': lr})
