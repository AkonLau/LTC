...
def set_outdir(conf):
    ...
    if 'timedir' in conf:
        ...
        # for auxiliary target coding regularization
        if conf.HTC is True:
            prefix += f'_HTC_gamma{conf.gamma_}_length{conf.code_length}'
        if conf.LTC is True:
            prefix += f'_LTC_gamma{conf.gamma_}_lambda{conf.lambda_}_beta{conf.beta_}_length{conf.code_length}' \
                      f'_margin{conf.margin_ratio}_{conf.active_type}'
        # for long-tailed datasets
        if conf.dataset == 'iNaturalist18' or conf.dataset == 'Imagenet-LT':
            prefix += '-'+conf.train_rule
